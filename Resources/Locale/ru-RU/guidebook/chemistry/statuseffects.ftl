reagent-effect-status-effect-Stun = оглушение
reagent-effect-status-effect-KnockedDown = нокдаун
reagent-effect-status-effect-Jitter = дрожь
reagent-effect-status-effect-TemporaryBlindness = слепота
reagent-effect-status-effect-SeeingRainbows = галлюцинации
reagent-effect-status-effect-Muted = немота
reagent-effect-status-effect-Stutter = заикание
reagent-effect-status-effect-ForcedSleep = бессознательное состояние
reagent-effect-status-effect-Drunk = опьянение
reagent-effect-status-effect-PressureImmunity = иммунитет к давлению
reagent-effect-status-effect-Pacified = невозможность атаковать
reagent-effect-status-effect-RatvarianLanguage = речь Ратвара
reagent-effect-status-effect-StaminaModifier = изменение выносливости
reagent-effect-status-effect-RadiationProtection = защита от радиации
reagent-effect-status-effect-Drowsiness = сонливость
reagent-effect-status-effect-NoScream = невосприимчивость к боли
reagent-effect-status-effect-Adrenaline = иммунитет к замедлению от повреждений