card-examined = Это {$target}.
cards-verb-shuffle = Перемешать
card-verb-shuffle-success = Карты перемешаны
cards-verb-draw = Взять карту
cards-verb-flip = Перевернуть карты
card-verb-join = Объединить карты
card-verb-organize-success = Карты перевернуты { $facedown ->
    [true]   рубашкой вверх
    *[false] лицом вверх
}
cards-verb-organize-up = Перевернуть карты лицом вверх
cards-verb-organize-down = Перевернуть карты рубашкой вверх
cards-verb-pickcard = Выбрать карту
card-stack-examine = { $count ->
    [one] В колоде {$count} карта.
    *[other] В колоде {$count} карт.
}
cards-stackquantitychange-added = Карта добавлена (Всего карт: {$quantity})
cards-stackquantitychange-removed = Карта удалена (Всего карт: {$quantity})
cards-stackquantitychange-joined = Колоды объединены (Всего карт: {$quantity})
cards-stackquantitychange-split = Колода разделена (Всего карт: {$quantity})
cards-stackquantitychange-unknown = Количество карт изменено (Всего карт: {$quantity})
cards-verb-convert-to-deck = Собрать в колоду
cards-verb-split = Разделить пополам

card-base-name = карта
card-deck-name = колода карт

card-sc-2-clubs = 2 треф
card-sc-3-clubs = 3 треф
card-sc-4-clubs = 4 треф
card-sc-5-clubs = 5 треф
card-sc-6-clubs = 6 треф
card-sc-7-clubs = 7 треф
card-sc-8-clubs = 8 треф
card-sc-9-clubs = 9 треф
card-sc-10-clubs = 10 треф
card-sc-ace-clubs = туз треф
card-sc-jack-clubs = валет треф
card-sc-king-clubs = король треф
card-sc-queen-clubs = дама треф

card-sc-2-diamonds = 2 бубен
card-sc-3-diamonds = 3 бубен
card-sc-4-diamonds = 4 бубен
card-sc-5-diamonds = 5 бубен
card-sc-6-diamonds = 6 бубен
card-sc-7-diamonds = 7 бубен
card-sc-8-diamonds = 8 бубен
card-sc-9-diamonds = 9 бубен
card-sc-10-diamonds = 10 бубен
card-sc-ace-diamonds = туз бубен
card-sc-jack-diamonds = валет бубен
card-sc-king-diamonds = король бубен
card-sc-queen-diamonds = дама бубен

card-sc-2-hearts = 2 червей
card-sc-3-hearts = 3 червей
card-sc-4-hearts = 4 червей
card-sc-5-hearts = 5 червей
card-sc-6-hearts = 6 червей
card-sc-7-hearts = 7 червей
card-sc-8-hearts = 8 червей
card-sc-9-hearts = 9 червей
card-sc-10-hearts = 10 червей
card-sc-ace-hearts = туз червей
card-sc-jack-hearts = валет червей
card-sc-king-hearts = король червей
card-sc-queen-hearts = дама червей

card-sc-2-spades = 2 пик
card-sc-3-spades = 3 пик
card-sc-4-spades = 4 пик
card-sc-5-spades = 5 пик
card-sc-6-spades = 6 пик
card-sc-7-spades = 7 пик
card-sc-8-spades = 8 пик
card-sc-9-spades = 9 пик
card-sc-10-spades = 10 пик
card-sc-ace-spades = туз пик
card-sc-jack-spades = валет пик
card-sc-king-spades = король пик
card-sc-queen-spades = дама пик

card-sc-joker = джокер

container-sealed = На объекте голографическая защитная пломба. При вскрытии пломба исчезнет.
container-unsealed = Пломба на объекте исчезла.